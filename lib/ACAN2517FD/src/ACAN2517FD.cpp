...
//----------------------------------- Install interrupt, configure external interrupt
  ...
  //----------------------------------- Wait (10 ms max) until requested mode is reached
    ...
    #ifdef ARDUINO_ARCH_ESP32
      /* Stack increased 1024→2048 words: 1024 overflows under heavy CAN traffic on ESP32.
         NOTE: do NOT pin to CPU0 — the ESP32 WiFi/BT stack runs on CPU0. Let FreeRTOS
         schedule ACAN2517Handler normally; loop() yields between iterations which feeds
         the IWDT. The ISR semaphore wakes this task; isr_poll_core() is never called
         directly from loop() so CPU1's tick is not blocked. */
      xTaskCreate (myESP32Task, "ACAN2517Handler", 2048, this, 16, &mESP32TaskHandle) ;
    #endif
...
void ACAN2517FD::isr_poll_core (void) {
  mSPI.beginTransaction (mSPISettings) ;
  /* ESP32 fix: taskDISABLE_INTERRUPTS() moved inside the loop so interrupts are
     re-enabled between iterations. The original code disabled interrupts for the
     entire drain loop, which causes an interrupt WDT panic when many frames are
     queued on a busy CAN bus (each SPI frame read takes ~50-100 us at 4 MHz,
     and the WDT fires if interrupts are masked for >~5 ms). */
      bool handled = true ;
        while (handled) {
        #ifdef ARDUINO_ARCH_ESP32
          taskDISABLE_INTERRUPTS () ;
